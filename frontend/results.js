...
function displayResults(data) {
    ...
    data.results.forEach(profile => {
        ...
        const makeLink = (url, text = 'View') => {
            if (!url || url === '—') return '—';
            return `<a href="${escapeHtml(url)}" target="_blank" rel="noopener" class="link-accent">${text}</a>`;
        };
        ...
        row.innerHTML = `
            ...
            <td>${profile.linkedin_url ? `<a href="https://${escapeHtml(profile.linkedin_url)}" target="_blank" rel="noopener" class="link-accent">${escapeHtml(linkedinDisplay)}</a>` : '—'}</td>
            <td>${profile.email ? `<a href="mailto:${escapeHtml(profile.email)}" class="link-accent">${escapeHtml(profile.email)}</a>` : '—'}</td>
            <td>${profile.phone ? escapeHtml(profile.phone) : '—'}</td>
            <td>${profile.website ? `<a href="https://${escapeHtml(profile.website)}" target="_blank" rel="noopener" class="link-accent">${escapeHtml(profile.website)}</a>` : '—'}</td>
