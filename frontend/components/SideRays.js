...
/**
 * SideRays — animated WebGL light-ray background (ported to plain JS from
 * react-bits' SideRays TS-CSS variant; rendering logic unchanged).
 *
 * Renders into a transparent canvas that fills its container. Pauses when
 * scrolled out of view via IntersectionObserver.
 */
  ...
  gl_FragColor = color;
            ...
            const loop = (t) => {
                ...
                try {
                    ...
                    // WebGL context lost — stop the loop
                }
            };
    ...
    return <div ref={containerRef} className={`${styles.sideRays} ${className}`.trim()} />;
};
